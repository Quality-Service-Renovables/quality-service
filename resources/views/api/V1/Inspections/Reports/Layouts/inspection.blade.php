    ...
    @foreach ($inspection->sections as $section)
        @if (has_results($section))
            ...
            <h3 class="primary-color uppercase" style="margin-bottom: 0px;margin-top:15px;">7.{{ $indexSection }}
                {{ $section['section_details']->ct_inspection_section }}</h3>
            <div class="text-center">
                <p>Tabla 7.{{ $indexSection }} {{ $section['section_details']->ct_inspection_section }}</p>
            </div>

            {{--  Campos  --}}
            @if (count($section['fields']))
                @foreach ($section['fields'] as $field)
                    @if ($field->content && ($field->content->inspection_form_value || $field->content->inspection_form_comments || $field->content->risk))
                        <table style="margin-bottom:10px;">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="border text-center" style="background-color: {!! $field->content->risk->ct_color ?? '' !!}">
                                        {!! $field->content->inspection_form_value ?? '' !!}</td>
                                    <td class="border">
                                        @isset($field->content->inspection_form_comments)
                                            {!! $field->content->inspection_form_comments ?? '' !!}
                                        @endisset
                                    </td>
                                </tr>
                                {{--  Sección campos evidencias --}}
                                @isset($field->content->evidences)
                                    @if (count($field->content->evidences) > 0)
                                        <tr>
            ...
            {{--  Subsecciones  --}}
            @if (isset($section['sub_sections']) && count($section['sub_sections']))
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="border text-center"
                                                    style="background-color: {!! $field->content->risk->ct_color ?? '' !!}">
                                                    {!! $field->content->inspection_form_value ?? '' !!}</td>
                                                <td class="border">
                                                    @isset($field->content->inspection_form_comments)
                                                        {!! $field->content->inspection_form_comments ?? '' !!}
